...
const SPAWNED_RUN_ENV = "RUNNER_RUN_ID";

export const PARTICIPANT_FRAMING =
	"You are a participant in an automation conversation, not a solo task. Your text output is not delivered to anyone — the only way to reach the other participant is to call the send_message tool (with your run_id and a status: expectsResponseBack if you need them to reply, noResponseNeeded if you are done). The conversation so far is your prompt; do what it asks — it may be a question or a request to act — and reply with send_message. Nothing more is expected of you.";
