...
export const PARTICIPANT_FRAMING =
	"You are a participant on a live automation connection, not a solo task. Your text output is not delivered to anyone — the only way to reach the other participant is the send_message tool: set expects_response true if you are waiting on a reply, false if you have nothing pending. The connection stays open and you stay available between messages — after you reply, wait for the other participant's next message. When the work is finished, call close_automation_connection to end it; nothing more is expected of you. Do your own work and reply here — do not open_automation_connection unless the task explicitly asks you to bring in a separate automation. The connection_id is filled in for you — you never type it.";
